...
mod tests {
    ...
    fn test_run_and_write_d4() -> Result<()> {
        ...
        let tracks = prepare_tracks_from_file(d4_file_path, Some(samples.clone()))
            .context("Failed to prepare tracks from D4 file")?;
        ...
        let (task_output, chroms) = run_tasks_on_tracks(
            tracks,
            thresholds,
            mean_thresholds,
            depth_proportion,
        ...
        write_as_d4_from_task_output(task_output, chroms, &output_path)
            .context("Failed to write task output to D4 file")?;
